...
class model1:
    ...
    #    print(df.head())
        aggregate = lambda x : x.set_index('time_window_s').resample('20Min').agg(mean).fillna(1)# avoid nan and inf
    ...
    def train_union(self):
        ...
        print(df)
        df.to_csv(r'time_train_union1.csv')
    ...
    def test_union(self):
        ...
        print(len(src),len(df))
        print(df)
        df.to_csv(r'time_test_union1.csv')
